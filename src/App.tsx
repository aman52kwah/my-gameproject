...
export default function App() {
  ...
  const speak = (text:any) => {
    if ('speechSynthesis' in window) {
      ...
      const kidVoice = voices.find(v => v.name.includes('Google') || v.name.includes('Karen') || v.name.includes('Moira'))
      ...
      setTimeout(() => {
        ...
        if (n < 5) {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              setN(1); setClicked([]); speak("Let's count again! One star!")
            }, 1500)
          }, 2000)
        }
      }, 3000)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-300 via-purple-300 to-blue-300 flex items-center justify-center p-4" onTouchStart={initAudio}>
      <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-2xl w-full">
          ...
          {Array.from({ length: n }, (_, i) => (
            <button
              ...
              className={`aspect-square rounded-3xl flex items-center justify-center transition-all active:scale-90 relative
                ${clicked.includes(i) ? 'bg-green-200' : 'bg-gray-100 shadow-2xl hover:shadow-xl'}`}
            >
              <Icon size={80} className={`${current.color} drop-shadow-2xl`} fill="currentColor" />
              {clicked.includes(i) && <span className="absolute text-6xl text-green-600 font-bold"></span>}
            </button>
          ))}
        </div>
        ...
        <div className="flex justify-center gap-4 mb-8">
          {[1,2,3,4,5].map(x => (
            <div key={x} className={`w-12 h-12 rounded-full transition-all shadow-lg
              ${x < n ? 'bg-green-500' : x === n ? 'bg-purple-600 scale-125 animate-pulse' : 'bg-gray-300'}`} />
          ))}
        </div>

        <div className="text-center">
          <button onClick={() => { 
            setN(1); setClicked([]); setCelebrate(false)
